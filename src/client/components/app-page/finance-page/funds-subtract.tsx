...
const FundsSubtractDialog:React.FunctionComponent<FundsSubtractDialogProps> = (props) => {
  const [state, setState] = React.useState<FundsSubtractDialogState>({
    step: 0,
    form: {},
    fields: {},
    loading: false
  ...
  const setInputRef:(label:keyof FundsSubtractDialogState['fields']) => React.Ref<HTMLInputElement> =
  ...
  const focusNext = (label:keyof FundsSubtractDialogState['fields']) => {
    ...
    ].map((inputs:(keyof FundsSubtractDialogState['fields'])[]) =>
      inputs.filter(label => {
  ...
  const focusInput = (label:keyof FundsSubtractDialogState['fields']) => {
    state.fields[label]?.focus()
  }

  const onChangeForm = (form:Partial<FundsSubtractDialogState['form']>) => {
    setState(state => ({
      ...state,
      form: {
        ...state.form,
  ...
  const next = async() => {
    ...
    if(state.step === 0 && store) {
      ...
      const {amount, from, datetime, remark, attachments} = state.form
                  ...
                  <CardContent classes={{root:classes.orderCardContent}}>
                    ...
                    <div style={{
                      ...
                      <div style={{
                        ...
                        <SubtractIcon width={16} height={16}/>
                        <Typography variant='h6' color='textPrimary'>
                          RM{displayCurrency(
                            form.amount
                              ? parseFloat(form.amount.replace(/[^0-9\.]/g, ''))
...
type FundsSubtractDialogProps = {
  store?: StoreDetailed
  onClose: () => void
}
type FundsSubtractDialogState = {
  step: number
  form: {
...
export default FundsSubtractDialog