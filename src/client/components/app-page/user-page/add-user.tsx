...
const AddUserDialog:React.FunctionComponent<AddUserDialogProps> = (props) => {
  const [state, setState] = React.useState<AddUserDialogState>({
    step: 0,
    form: {},
    fields: {},
    loading: false
  })
  const [, {createUser, displayUsers}] = useUserState()
  ...
  const setInputRef:(label:keyof AddUserDialogState['fields']) => React.Ref<HTMLInputElement> =
  ...
  const focusNext = (label:keyof AddUserDialogState['fields']) => {
    const inputs = [
  ...
  const focusInput = (label:keyof AddUserDialogState['fields']) => {
    state.fields[label]?.focus()
  }
  const onChangeForm = (form:Partial<AddUserDialogState['form']>) => {
    setState(state => ({
      ...state,
      form: {
        ...state.form,
    ...
    if(state.step === 0) {
      ...
      try {
        await createUser({
          ...
          mobileNumber: `+60${mobileNumber?.replace(/\D/g, '')}`,
          email: email?? '',
          address: address !== ''
            ? address
            : undefined
            ...
            <div className={classes.content}>
              <div className={classes.titleSection}>
                ...
                <Typography variant='body1' color='textSecondary' gutterBottom>
                  Your add user request is done successfully
                </Typography>
              </div>
            </div>
          </SwipeableViews>
