...
const UpdateProductsDialog:React.FunctionComponent<UpdateProductsDialogProps> = (props) => {
  const [state, setState] = React.useState<UpdateProductsDialogState>({
    step: 0,
    form: {},
    fields: {},
    loading: false
  ...
  const [, {updateProducts}] = useRequestState()
  ...
  const setInputRef:(label:keyof UpdateProductsDialogState['fields']) => React.Ref<HTMLInputElement> =
  ...
  const onChangeForm = (form:Partial<UpdateProductsDialogState['form']>) => {
    setState(state => ({
      ...state,
      form: {
        ...state.form,
    ...
    if(state.step === 0) {
      ...
      try {
        await updateProducts({
          ...
          products: chosenProducts?.flatMap(({id, quantity, price}) => {
            return typeof quantity === 'number' && typeof price === 'number'
              ? [{id, quantity, price}]
              : []
          })?? []
        })
...
type UpdateProductsDialogProps = {
  request?: OrdersRequestDetailed
  onClose: () => void
}
type UpdateProductsDialogState = {
  step: number
  form: {
    products?: {
      id: string
...
export default UpdateProductsDialog