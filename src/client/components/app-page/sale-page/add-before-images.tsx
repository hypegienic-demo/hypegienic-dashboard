...
const AddBeforeImagesDialog:React.FunctionComponent<AddBeforeImagesDialogProps> = (props) => {
  const [state, setState] = React.useState<AddBeforeImagesDialogState>({
    step: 0,
    form: {},
    loading: false
  })
  const [, {addBeforeImages}] = useRequestState()

  React.useEffect(() => {
    setTimeout(() => state.updateHeight?.())
  }, [state.updateHeight, state.form.imagesBefore])

  const onChangeForm = (form:Partial<AddBeforeImagesDialogState['form']>) => {
    setState(state => ({
      ...state,
      form: {
        ...state.form,
          ...
          >
            <div className={classes.content}>
              ...
              <div className={classes.formFields}>
                <FileField
                  label='Images Before'
                  values={form.imagesBefore}
                  onChange={imagesBefore => onChangeForm({imagesBefore})}
                />
              </div>
            </div>
            <div className={classes.content}>
              <div className={classes.titleSection}>
                ...
                <Typography variant='body1' color='textSecondary'
                  gutterBottom
                >
                  {order?.id} {order?.name} to delivered store
                </Typography>
              </div>
              <div className={classes.formFieldCards}>
                <Card variant='outlined'>
                  <div style={{
                    ...
                    {form.imagesBefore?.map((image, index) => {
...
type AddBeforeImagesDialogProps = {
  order?: OrderDetailed
  onClose: () => void
}
type AddBeforeImagesDialogState = {
  step: number
  form: {
    imagesBefore?: File[]
  }
...
export default AddBeforeImagesDialog