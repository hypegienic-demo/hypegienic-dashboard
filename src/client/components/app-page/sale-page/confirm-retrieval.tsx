...
const ConfirmRetrievalDialog:React.FunctionComponent<ConfirmRetrievalDialogProps> = (props) => {
  const [state, setState] = React.useState<ConfirmRetrievalDialogState>({
    loading: false
  })
  const [, {confirmRetrieve}] = useRequestState()
          ...
          <div className={classes.content}>
            <div className={classes.titleSection}>
              ...
              <Typography variant='body1' color='textSecondary'
                gutterBottom
              >
                {order?.id} {order?.name} to retrieved back
              </Typography>
            </div>
          </div>
        </DialogContent>
...
type ConfirmRetrievalDialogProps = {
  order?: OrderDetailed
  onClose: () => void
}
type ConfirmRetrievalDialogState = {
  loading: boolean
  error?: {
    open: boolean
    message: string
